        ...
        <S.MobileView {...settings}>
          {buildingData.imageUrls !== undefined && buildingData.imageUrls.length > 0 ? (
            buildingData.imageUrls.map((el) => (
              <figure key={el}>
                <Image src={el} alt="buildImg" fill style={{ objectFit: "cover" }} unoptimized />
              </figure>
            ))
          ) : (
            <figure>
        ...
        <S.PCView>
          <figure className="mainImg">
            {buildingData.imageUrls?.[0] !== undefined ? (
              <Image src={buildingData.imageUrls?.[0]} alt="buildImg" fill style={{ objectFit: "cover" }} unoptimized />
            ) : (
