...
import type { IGeocodeAPIReturn, IWriteForm } from "@/src/commons/types";
...
export const useAddressSearch = (setValue: UseFormSetValue<IWriteForm>, getValues: UseFormGetValues<IWriteForm>, onModalToggle: () => void): IUseAddressSearchProps => {
  ...
  const [geocodeData, setGeocodeData] = useState<IGeocodeAPIReturn | null>(null);
  const currentAddress = getValues("address");

  const fetchGeocode = async (address: string): Promise<void> => {
    try {
      const response = await axios.get<IGeocodeAPIReturn>(`/api/fetchSelectGeocode?address=${encodeURIComponent(address)}`);
