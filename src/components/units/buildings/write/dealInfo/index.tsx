...
export default function DealInfo(props: IDealInfoProps): JSX.Element {
  const { register, isEdit, docData } = props;

  return (
    <section>
      <UnderlineTitle label="거래 정보" />
      <div className="inputUnit">
        <TextFieldBasic required role="input-price" type="number" label="매매가" defaultValue={isEdit ? docData?.price : ""} register={register("price")} />
        <BasicUnit label="만원" />
      </div>
      <div className="inputUnit">
        <TextFieldBasic required role="input-manageCost" type="number" label="관리비" defaultValue={isEdit ? docData?.manageCost : ""} register={register("manageCost")} />
        <BasicUnit label="만원" />
      </div>
    </section>
  );
