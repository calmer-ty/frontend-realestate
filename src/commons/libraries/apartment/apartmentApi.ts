...
export const apartmentApi = async (region: IRegionItem): Promise<IApartment> => {
  // console.log("[region] ", region);
  const currentDate = getCurrentDate();
  // 캐시에 없는 경우 실제 데이터를 요청합니다
  const apartmentUrl = `https://apis.data.go.kr/1613000/RTMSDataSvcAptTrade/getRTMSDataSvcAptTrade?serviceKey=${API_KEY}&LAWD_CD=${region.region_cd}&DEAL_YMD=${currentDate}&pageNo=1&numOfRows=10`;
