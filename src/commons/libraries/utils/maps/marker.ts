...
export const markerIconContent = (data: IApartmentItem, firestoreDatas: IFirestore[]): string => {
  const matchedFirestoreData = firestoreDatas.find(
    (firestoreData) => firestoreData.address === getShortenedCityName(data.estateAgentSggNm ?? DEFAULT_STRING_VALUE)
  ...
  const price = data.dealAmount;
  // price = price.endsWith(".0") ? price.slice(0, -2) : price;

  // 공통 스타일
  const area = `<div style="${matchedFirestoreData !== undefined ? markerStyle.topAreaActive : markerStyle.topArea}">${Math.round((data.excluUseAr ?? DEFAULT_NUMBER_VALUE) * 0.3025)}평</div>`;
