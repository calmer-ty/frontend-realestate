...
export default async function fetchAllGeocode(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  const { buildingType, regionName, regionCode } = req.query; // 쿼리 문자열에서 buildingType을 추출합니다.

  if (typeof buildingType !== "string" || typeof regionName !== "string" || typeof regionCode !== "string") {
    res.status(400).json({ error: "Invalid buildingType" });
    return;
  }

  try {
    const data = await getAllGeocodeData({ buildingType, regionName, regionCode }); // 지역 데이터 처리 함수를 호출하고 결과를 기다립니다
